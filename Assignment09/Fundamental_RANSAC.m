...
%     end
    ...
    if iter  > (N)       
        [normPts1, T1] = normalizePts(p(:,finalIdx));
        [normPts2, T2] = normalizePts(q(:,finalIdx));
        ...
        disp(finalF)
        % compute error
        [~, dist] = sampsonError(finalF, pH, qH);
        finalIdx = find(dist<thresh);
        disp('final inlier')
        disp(length(finalIdx))
        disp('optimized H')
        disp(finalF)
...
% eptemp = q'*F.*p';
% ep = sum(eptemp,2);
% Jtemp = zeros([length(p),4]);
% for i = 1: length(p)
% Jtemp(i,:) = [F(1,1)*q(1,i) + F(2,1)*q(2,i) + F(3,1),...
%      F(1,2)*q(1,i) + F(2,2)*q(2,i) + F(3,2),...
%      F(1,1)*p(1,i) + F(1,2)*p(2,i) + F(1,3),...
%      F(2,1)*p(1,i) + F(2,2)*p(2,i) + F(2,3)];
% end
% J = sum(Jtemp.*Jtemp,2);
% error = ep.^2 ./ J;
end
...
%% OLD
% function distance = dist(F,p,q)
% distance = zeros(1,length(p));
% for i = 1 : length(p)
%     lineEq = F*p(:,i);
%     distance(1,i) = abs(lineEq.' .* q(:,i)) / sqrt(lineEq(1).^2 +lineEq(2).^2);
% end
% 
% end